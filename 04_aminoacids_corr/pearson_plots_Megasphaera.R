#!/usr/bin/env Rscript
## Purpose of script: pearson correlation and network
...
# Parameters of pearson correlation
...
pearson_table <- melted_cormat %>% filter(value != 0) %>% filter(Var1 != Var2)

write.table(pearson_table,
    file =  args[2],
    sep = "\t")

try(for(row in 1:nrow(pearson_table)){
    x <- paste0(pearson_table$Var1[row])
    y <- paste0(pearson_table$Var2[row])
    pearson_plot <-
    ggplot(table,
        aes(x = get(x),
        y = get(y)),
        fill = TRUE) +
    geom_point() +
    geom_point(color = c("blue", "blue", "red", "red",
                        "red")) +
