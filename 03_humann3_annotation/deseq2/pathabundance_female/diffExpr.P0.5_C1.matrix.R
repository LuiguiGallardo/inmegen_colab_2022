...
if (file.exists("diffExpr.P0.5_C1.matrix.RData") && ! NO_REUSE) {
    print('RESTORING DATA FROM EARLIER ANALYSIS')
    load("diffExpr.P0.5_C1.matrix.RData")
} else {
    print('Reading matrix file.')
    primary_data = read.table("diffExpr.P0.5_C1.matrix", header=T, com='', row.names=1, check.names=F)
    primary_data = as.matrix(primary_data)
}
source("/home/luigui/apps/trinityrnaseq-Trinity-v2.5.1/Analysis/DifferentialExpression/R/heatmap.3.R")
source("/home/luigui/apps/trinityrnaseq-Trinity-v2.5.1/Analysis/DifferentialExpression/R/misc_rnaseq_funcs.R")
source("/home/luigui/apps/trinityrnaseq-Trinity-v2.5.1/Analysis/DifferentialExpression/R/pairs3.R")
source("/home/luigui/apps/trinityrnaseq-Trinity-v2.5.1/Analysis/DifferentialExpression/R/vioplot2.R")
data = primary_data
myheatcol = colorpanel(75, 'purple','black','yellow')
samples_data = read.table("../metadata_female.txt", header=F, check.names=F, fill=T)
...
write.table(data, file="diffExpr.P0.5_C1.matrix.log2.centered.dat", quote=F, sep='	');
if (nrow(data) < 2) { stop("
...
write.table(sample_cor, file="diffExpr.P0.5_C1.matrix.log2.centered.sample_cor.dat", quote=F, sep='	')
sample_dist = dist(t(data), method='euclidean')
hc_samples = hclust(sample_dist, method='complete')
pdf("diffExpr.P0.5_C1.matrix.log2.centered.sample_cor_matrix.pdf")
...
", "diffExpr.P0.5_C1.matrix.log2.centered") , ColSideColors=sampleAnnotations, RowSideColors=t(sampleAnnotations))
dev.off()
gene_cor = NULL
...
heatmap_data = data
pdf("diffExpr.P0.5_C1.matrix.log2.centered.genes_vs_samples_heatmap.pdf")
...
", "diffExpr.P0.5_C1.matrix.log2.centered" ) , ColSideColors=sampleAnnotations)
dev.off()
save(list=ls(all=TRUE), file="diffExpr.P0.5_C1.matrix.RData")
